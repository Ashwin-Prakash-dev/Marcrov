...
# Assuming you create this helper function in src/data_pipeline.py
# from src.data_pipeline import apply_exponential_decay
    ...
    for i in range(len(series)):
        if pd.notna(series.iloc[i]):
            ...
            if days_stale > max_staleness:
                ...
                # Exponential decay formula: N(t) = N(0) * (0.5)^(t/halflife)
...
def test_exponential_decay_halflife():
    ...
    # At day 10 (index 10), exactly one halflife has passed. Value MUST be 50.0
...
def test_max_staleness_cutoff():
    ...
    # Day 46 exceeds max staleness, must return NaN
